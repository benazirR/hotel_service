...
class RoomDAO(BaseDAO):
    ...
    async def find_all(cls, hotel_id: int, date_from: date, date_to: date):
        booked_rooms = (
            select(Bookings.room_id, func.count(Bookings.room_id).label("rooms_booked")).select_from(Bookings).where(
                or_(
                    and_(
        ...
        get_rooms = (
            ...
            ).join(booked_rooms, booked_rooms.c.room_id == Rooms.id, isouter=True).where(
